...
- **CON-128** After installing or verifying Praxis, Conditor MUST check whether the target repository carries the Praxis provenance contract: a `ros.json` `provenance` policy with `enforce: true` and a `requiredFrom` date (Praxis RQ-ROS-2026-A007), the AGENTS.md agent identity/provenance guidance (RQ-ROS-2026-A012), detected tolerantly (a stable marker, a heading naming identity and provenance, or the key phrases) so that rewording in a Praxis release does not fail verification, and `docs/agent-provenance.md`. The check MUST report one of `enabled`, `not-supported-by-installed-version`, or `missing-when-expected`. `missing-when-expected` MUST fail verification before lock state is written (CON-010). Conditor MUST NOT write or repair Praxis-owned provenance files itself (CON-020).
...
- **CON-130** Conditor's own Praxis calls (mission capture, readiness transition, start) MUST identify Conditor as automation: `ROS_ACTOR_KIND=automation`, `ROS_ACTOR=conditor`, `ROS_TELEMETRY_RUNTIME=conditor`, while keeping `--actor conditor` for Praxis releases that predate the identity variables (RQ-ROS-2026-A006, RQ-ROS-2026-A016). Before declaring its own identity, Conditor MUST remove every variable in the Praxis identity-environment list (contract revision 1.1: `ROS_*` identity and `ROS_TELEMETRY_*`, runtime session ids, `GITHUB_ACTIONS`, `GITHUB_RUN_ID`, `OLLAMA_HOST`). No outer session, run, model, or provider may leak into Conditor's identity, and no provider may be inferred from CI or local-model signals.
- **CON-131** A launched agent MUST receive only true, known identity: `ROS_ACTOR_KIND=agent` and the provider/runtime of the CLI Conditor launches (`codex` → `openai`/`codex`, `claude` → `anthropic`/`claude-code`), and `ROS_TELEMETRY_MODEL` only when `execution.model` is configured, in which case the same model is passed to the CLI. The agent MUST NOT receive Conditor's identity or any inherited variable from the Praxis identity-environment list, such as `ROS_ACTOR`, `ROS_EXECUTION_ID`, `CLAUDE_CODE_SESSION_ID`, or `GITHUB_RUN_ID`. It begins its own Praxis execution, and the operator's session or CI run must not be merged into it. Conditor MUST NOT fabricate provider, model, or runtime values (RQ-ROS-2026-A012, RQ-ROS-2026-A016).
...
|---|---|---|
| CON-128 | `src/Conditor.Core/PraxisProvenance.fs` (`assess`, `observe`, `inspect`, tolerant `hasAgentGuidance`); gate in `src/Conditor.Core/Installer.fs` (`execute`); `Doctor.provenanceFinding`; CLI output in `src/Conditor.Cli/Program.fs` (`run`) | `tests/Conditor.Tests/Program.fs` "Praxis provenance readiness" (fixture `tests/Conditor.Tests/fixtures/praxis-provenance/` copied from Praxis `c2657ef` with SHA-256 check; guidance-wording tolerance tests) |
...
| CON-130 | `src/Conditor.Core/AgentIdentity.fs` (`inheritedIdentityVariables`, `conditorRosEnvironment`); `Mission.runRos` via `ProcessRunner.runProcessWithEnvironment` | "Identity propagation" tests: automation, conditor actor, cleared list equals vendored `identity-environment.json` (Praxis c2657ef), no inherited session/CI/local-model signal |
